...
if (!isset($_POST['cancel_all']) && !isset($_POST['cancel_one'])&& !isset($_POST['done_all']) && !isset($_POST['done_one'])){
...
if(isset($_POST['cancel_all'])){
	try{
		...
		foreach($_POST['Orders'] as $o_id){
			...
			if ($result[0]['o_state'] == 'cancel' || $result[0]['o_state'] == 'finished'){
		...
		foreach($_POST['Orders'] as $o_id){
			...
			$_SESSION['u_money'] = $newMoney; //更改目前在 session 的使用者的錢包的錢
...
}else if(isset($_POST['cancel_one'])){
	try{
	...
	try{
		...
		foreach($_POST['Orders'] as $o_id){
			...
			$sql = "update orders set o_state = :o_state, o_end_time =:time where OID =:o_id";
			$stmt=$conn->prepare($sql);
			$stmt->execute(array('o_state' => $state, 'time'=> $time, 'o_id'=>$o_id));
		}
		# 重新設定session
		echo <<< EOT
		<!DOCTYPE html>
...
}else if(isset($_POST['done_one'])){
	try{
		$OID = $_POST['done_order'];

		# 0.先確定這筆訂單還沒有被完成或者被另一方取消，也就是 o_state =  not finish，否則報錯
		$stmt = $conn->prepare("select o_state from orders where OID =:o_id");
		$stmt->execute(array ('o_id'=> $OID));
		$result = $stmt->fetchALL();
		if ($result[0]['o_state'] == 'cancel'){
			throw new Exception('the order had been canceled');
		}else if($result[0]['o_state'] == 'finished'){
